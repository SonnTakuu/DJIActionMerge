...
## Set Path
SOURCE_DEVICE=$(find /storage -type f -name "Fv-AutoMerge-Ready.log" | sed 's|/Fv-AutoMerge-Ready.log||')
if [ -z "$SOURCE_DEVICE" ]; then
    echo -e "${BLACK_BG_GREEN_TEXT}[ERROR] Source device with Fv-AutoMerge-Ready.log not found!${RESET}"
    exit 1
fi
printf "${BLACK_BG_GREEN_TEXT}[Info] Source device: %s${RESET}\n" "$SOURCE_DEVICE"

# Set Destination USB devices
...
if [ "$answer" != "${answer#[Yy]}" ]; then
    ...
    DEST_DEVICE=$(find /storage -type f -name "Fv-Merge-Destination.log" | sed 's|/Fv-Merge-Destination.log||')
    echo -e "${BLACK_BG_GREEN_TEXT}..........[ DONE ]${RESET}"

    # Verify destination device is found
...
MERGE_PATH="$DEST_DEVICE/Fv-MediaFolder/Merged"
...
# Set size threshold for 3.5 GB (in bytes)
...
# Function to get file date and time (Termux uses `stat -c`)
get_file_datetime() {
    stat -c "%y" "$1" | awk '{print $1"_"substr($2, 1, 5)}' | sed 's/[-:]//g' | sed 's/_//g'
}
